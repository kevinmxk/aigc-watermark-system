...
- **三级水印生成**：输入文本提示词 + 用户ID，自动完成 Tree-Ring 频域水印 → DAAM 语义掩码 → StegaStamp 用户指纹的三级注入流程
- **语义感知强度调制（Guided Strength）**：DAAM 热力图自适应二值化为语义掩码，根据语义覆盖率 r 动态降低水印强度 w' = w₀ × (1 − α × r)，保护语义区域视觉质量
- **水印检测与验证**：支持单张图像和批量检测，逐层验证 Tree-Ring 频域水印、DAAM 语义特征和 StegaStamp 用户ID
- **BCH + CRC 纠错编码**：用户字符串经 CRC-16 校验 + BCH 纠错编码为 100 比特，支持纠正 3~8 比特错误，大幅提升 StegaStamp 在 JPEG 压缩、噪声等场景下的识别率
- **批量生成**：从文本文件读取多条提示词，自动批量生成带水印图像
- **Streamlit Web 界面**：提供可视化操作界面，支持单张生成、批量生成、水印检测、系统架构展示

### 达到的效果

| 指标 | 效果 |
|------|------|
| Tree-Ring 检测准确率 | 100%（30/30 盲测全对） |
| 图像质量 | FID=18.7，SSIM=0.92 |
| StegaStamp 用户ID容错 | BCH 纠错后支持 3~8 bit 错误恢复 |
| 环境兼容 | 通过分阶段执行隔离三个不兼容环境（PyTorch + TensorFlow） |

### 适用场景

- AI 生成图像的**品牌保护**（Tree-Ring 频域水印）
- 内容**来源验证**（DAAM 语义感知掩码）
- 用户级**版权溯源**（StegaStamp 用户指纹）
- AIGC 平台的水印嵌入与检测服务
...
| **作者单位** | 暨南大学 · 网络空间安全学院 |
| **开发语言** | Python |
| **框架** | PyTorch + diffusers 0.21.2+Stable Diffusion 2.1 Base |
